...
def run_act(cfg: Config, date: str) -> None:
    # Concurrent auto-Act sessions SERIALIZE via a non-blocking session lock (#299
    # review round 11): two flows completing at once both pass act_due before either
    # advances the marker, and the frontier union cannot undo two leaves appending
    # duplicate act-log entries over the same snapshot. The whole session holds
    # `.act-session.lock` (never the marker lock — mark_reviewed re-acquires that,
    # and revalidate must not block for a whole interactive review); the loser skips
    # loudly, and its unreviewed cycles stay in scope for the next due Act.
    cfg.process_dir.mkdir(parents=True, exist_ok=True)
    session = cfg.process_dir / ".act-session.lock"
    try:
        fh = session.open("w")
    except OSError as exc:
        print(f"leaves: cannot open the Act session lock ({exc}); Act skipped this "
              "run — its cycles stay unreviewed", file=sys.stderr)
        return
    try:
        try:
            worktree._lock_file(fh, wait=False)
        except OSError:
        ...
        try:
            ...
            # runs — even if a command-mode Act judged "no delta" and wrote no
            # act-log entry, the review happened, over exactly the pre-session
            # snapshot. delta_guard applies the mid-session delta protection INSIDE
            # the marker's critical section (#299 review round 7 — a scan out here
            # would race revalidate's unmark_reviewed); the stamp's `changed`
            # verdict decides, so a confirming revalidation doesn't withhold.
            act_mod.mark_reviewed(cfg, reviewed=covered, date=date,
                                  delta_guard=started)
        finally:
