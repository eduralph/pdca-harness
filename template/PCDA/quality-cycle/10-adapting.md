...
4. **Don't re-raise standing reds every run.** Wrap a whole-suite gate in a **baseline-diff**: parse its result, diff against a checked-in baseline manifest, exit non-zero only on a *delta* (a new failing test, or a cleared baseline red). Otherwise the reviewer + human re-diagnose the same reds every cycle.
